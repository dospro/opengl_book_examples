...
class MovingCamera : public SDLApp {
public:
    MovingCamera(int width, int height);
    /**
    ...
    void onKeyPress(unsigned int scancode) override;
...
MovingCamera::MovingCamera(int width, int height) : SDLApp(width, height) {
    init();
}

void MovingCamera::setup_vertices() {
...
std::string MovingCamera::load_shader(const std::string &filename) {
...
GLuint MovingCamera::create_shader_program() {
...
void MovingCamera::onKeyPress(unsigned int scancode) {
    const float mov = 0.05;
...
void MovingCamera::update(double delta_time) {
    glClearColor(0.0, 0.0, 0.0, 0.0);
    glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
    glUseProgram(rendering_program);

    // Vamos a mandar x_rot y y_rot al shader
    GLint loc_x_rot = glGetUniformLocation(rendering_program, "x_rot");
    glUniform1f(loc_x_rot, x_rot);
    GLint loc_y_rot = glGetUniformLocation(rendering_program, "y_rot");
    glUniform1f(loc_y_rot, y_rot);

    // Ahora mandamos la posición del cubo
    m_mat = Matrix::make_translate(0.0, -2.0, 0.0);
    GLint loc_model_matrix = glGetUniformLocation(rendering_program, "model_matrix");
    glUniformMatrix4fv(loc_model_matrix, 1, GL_TRUE, m_mat.getData());

    // Y finalmente la matriz de proyeccion
    ...
    // x_rot y y_rot no han cambiado por lo que no hay que mandarlo de nuevo
    // pero si hay que mandar la posición de la piramide
...
int main() {
    auto *app = new MovingCamera(600, 600);
    app->run();
    return 0;
}